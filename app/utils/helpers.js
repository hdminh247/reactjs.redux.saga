...
export const calculateDirection = (bookingData, callBackChangeStore) => {
  ...
  // save result is {}
  if (destinationArr.length === 0 || (destinationArr.length >= 1 && destinationArr.filter(des => _.isEmpty(des.address)).length > destinationArr.length - 1)) {
    callBackChangeStore("directions", {});
    return;
  }
  ...
  // waypoints list is item in destinationArr
  let waypoints = [];
  destinationArr.forEach((des, index) => {
    if (index < destinationArr.length)
      waypoints.push({
...
export function makeEvent(eventName) {
  console.log("1. MAKE EVENT NAME SOCKET....: ", eventName);
  // Create a new event
...
export const formatRichTextToOnlyChar = (str) => {
  //remove tag all html, remove all \n
  return str.replace(/<[^>]*>?/gm, "").replace(/\n/g, "");
};


export const isSureDriver = (user) => {
  const { company = { isApproved: false } } = user;
  console.log("isSureDriver--------------", user);
  return company && company.isApproved;
};
